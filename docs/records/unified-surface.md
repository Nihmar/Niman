...
| WYSIWYG size cap | ~~**200 KB** (`_maxWysiwygBytes`)~~ — **gone, 2026-09-21** | `wysiwyg_editor.dart` | A mode-specific cap is exactly the non-uniformity R3 forbids. Removed before phase 4 rather than with it: with the cap in place the *stress test* could not be run, because the note that stresses the surface is the note it refused — and the work it was refusing (a ~2 s conversion) was being paid anyway, since the codec decodes before the build that refuses (#254). |
